...
contract TaxAuthorityV2 {
    ...
    // NEW: allowlist for bank-callers (MVP safety valve)
    mapping(address => bool) public trustedBankCaller;
    ...
    event TrustedBankCallerSet(address indexed caller, bool ok);
    event Declared(uint32 indexed year, bytes32 indexed taxpayerAccountId, bytes32 indexed republicKey, uint256 profit, uint256 taxDue);
    event TaxPaid(uint32 indexed year, bytes32 indexed taxpayerAccountId, bytes32 indexed republicKey, uint256 amount, uint256 repPart, uint256 confPart, uint256 totalPaid);
    ...
    modifier onlyAllowedBankCaller() {
        // accept if explicitly trusted OR licensed
        if (!trustedBankCaller[msg.sender]) {
            require(licenseRegistry.isLicensed(msg.sender), "NOT_LICENSED_BANK");
        }
        _;
    }

    constructor(address ledgerAddr, address licenseRegAddr, bytes32 confedBudgetAccountId) {
        owner = msg.sender;
    ...
    function setTrustedBankCaller(address caller, bool ok) external onlyOwner {
        trustedBankCaller[caller] = ok;
        emit TrustedBankCallerSet(caller, ok);
    }
    ...
    function declareProfit(uint32 year, bytes32 taxpayerAccountId, bytes32 republicKey, uint256 profit) external {
    ...
    function payTax(
        ...
        uint256 amount,
        bytes32 memo
    ) external onlyAllowedBankCaller {
        Record storage r = recordOf[year][taxpayerAccountId];
